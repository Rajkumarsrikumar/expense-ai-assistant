-- Project Tracker Schema - Paste this in Supabase SQL Editor and click Run
...
CREATE POLICY "history_select_own" ON public.history_entries FOR SELECT USING (
  user_id = auth.uid() AND EXISTS (SELECT 1 FROM public.projects p WHERE p.id = history_entries.project_id AND p.user_id = auth.uid())
);
DROP POLICY IF EXISTS "history_insert_own" ON public.history_entries;
CREATE POLICY "history_insert_own" ON public.history_entries FOR INSERT WITH CHECK (
  user_id = auth.uid() AND EXISTS (SELECT 1 FROM public.projects p WHERE p.id = project_id AND p.user_id = auth.uid())
);

CREATE OR REPLACE FUNCTION public.update_projects_updated_at() RETURNS TRIGGER AS $$
...
CREATE OR REPLACE FUNCTION public.auto_insert_project_history() RETURNS TRIGGER AS $$
BEGIN
  IF OLD.status IS DISTINCT FROM NEW.status THEN
    INSERT INTO public.history_entries (user_id, project_id, type, from_value, to_value) VALUES (NEW.user_id, NEW.id, 'status_change', OLD.status, NEW.status);
  END IF;
  IF OLD.progress IS DISTINCT FROM NEW.progress THEN
    INSERT INTO public.history_entries (user_id, project_id, type, from_value, to_value) VALUES (NEW.user_id, NEW.id, 'progress_update', OLD.progress::text, NEW.progress::text);
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
DROP TRIGGER IF EXISTS trigger_auto_project_history ON public.projects;
CREATE TRIGGER trigger_auto_project_history AFTER UPDATE ON public.projects FOR EACH ROW
